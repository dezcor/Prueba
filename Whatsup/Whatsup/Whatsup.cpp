#include "stdafx.h"  //________________________________________ Whatsup.cpp
#include "Whatsup.h"
...
BOOL WINAPI GetExtensionVersion(HSE_VERSION_INFO* pVer)
{
	...
	strcpy_s(pVer->lpszExtensionDesc, HSE_MAX_EXT_DLL_NAME_LEN, "Whatsup ISAPI Extension");
...
void SelectWindow(const wchar_t* windowID, Web::HttpConnector& h)
{
	...
	else if(wcscmp(windowID, L"DeleteMessage") == 0) {DeleteMessage page; page.Run(h);}
	else if(wcscmp(windowID, L"MessageList") == 0) {MessageList page; page.Run(h);}
	else if(wcscmp(windowID, L"SendMessage") == 0) {SendMessagex page; page.Run(h);}
	else if(wcscmp(windowID, L"ContactList") == 0) {ContactList page; page.Run(h);}
	else if(wcscmp(windowID, L"Index") == 0) {Index page; page.Run(h);}
	else if(wcscmp(windowID, L"NewUser") == 0) {NewUser page; page.Run(h);}
	else if(wcscmp(windowID, L"AddContacto") == 0) {AddContacto page; page.Run(h);}
